...
- **Client SDK:** [NexKeyRuntime](https://github.com/ciqueira/NexKeyRuntime)
  ...
  verification of activation certificates. macOS and Windows. Its contract is
  Apache-2.0; the license for the compiled binaries is still a draft, so
  third-party use is not yet cleared.
- **Offline verification:** a licensed product does not contact a server to
  run. The SDK verifies the activation certificate against a public keyring
  embedded in the product, on the machine, with no network. The network renews
  a certificate; it never grants one that is already valid.
- **Published wire formats:** ProductData, the activation certificate, and the
  update manifest are published as [JSON Schemas](https://github.com/ciqueira/NexKeyRuntime/tree/main/schemas)
  under Apache-2.0, so an integrator can verify what the SDK accepts and
  reproduce it independently.
- **Developer-held recovery authority:** a recovery key generated by the
  developer, whose private half never reaches Nexus infrastructure and whose
  public half travels in the product's keyring. It allows certificates to be
  issued for already-installed copies with no Nexus infrastructure involved.
  Mechanism implemented; end-to-end verification is not complete — see
